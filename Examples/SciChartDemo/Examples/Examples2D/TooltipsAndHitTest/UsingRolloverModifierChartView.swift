...
class UsingRolloverModifierChartView: SingleChartLayout {
    ...
    override func initExample() {
        ...
        rs1.strokeStyle = SCISolidPenStyle(colorCode: 0xFFa1b9d7, thickness: 1)

        let rs2 = SCIFastLineRenderableSeries()
        rs2.dataSeries = ds2
        rs2.pointMarker = ellipsePointMarker
        rs2.strokeStyle = SCISolidPenStyle(colorCode: 0xFF0b5400, thickness: 1)
        
        let rs3 = SCIFastLineRenderableSeries()
        rs3.dataSeries = ds3
        rs3.strokeStyle = SCISolidPenStyle(colorCode: 0xFF386ea6, thickness: 1)

        SCIUpdateSuspender.usingWith(surface) {
            self.surface.xAxes.add(xAxis)
            self.surface.yAxes.add(yAxis)
            self.surface.renderableSeries.add(items: rs1, rs2, rs3)
