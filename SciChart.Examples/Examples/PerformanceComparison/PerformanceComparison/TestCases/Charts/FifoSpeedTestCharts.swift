...
//
// FifoSpeedTestCharts.swift is part of the SCICHART® PerformanceComparison.
...
class FifoSpeedTestCharts: ChartsSpeedTestBase {
    ...
    override func updateChart(testParameters: TestParameters) {
        _ = chartDataSet.remove(at: 0)
        chartDataSet.append(ChartDataEntry(x: xCount, y: randomWalkGenerator.next(min: 0.0, max: 1.0)))
        xCount += 1
