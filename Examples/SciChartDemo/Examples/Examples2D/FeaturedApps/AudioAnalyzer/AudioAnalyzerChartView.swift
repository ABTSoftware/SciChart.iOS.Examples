...
class AudioAnalyzerChartView: SCDAudioAnalyzerLayout {
    ...
    private func initAudioStreamChart() {
        ...
        rs.dataSeries = audioDS
        rs.strokeStyle = SCISolidPenStyle(color: .gray, thickness: 1.0)
        ...
        SCIUpdateSuspender.usingWith(audioStreamChart) {
            ...
            self.audioStreamChart.renderableSeries.add(items: rs)
        }
    }
    ...
    private func initFFTChart() {
        ...
        let rs = SCIFastColumnRenderableSeries()
        rs.dataSeries = fftDS
        rs.zeroLineY = minDB
        rs.paletteProvider = FFTPaletteProvider()
        
        SCIUpdateSuspender.usingWith(fftChart) {
            self.fftChart.xAxes.add(items: xAxis)
            self.fftChart.yAxes.add(items: yAxis)
            self.fftChart.renderableSeries.add(rs)
        }
    }
    ...
    private func initSpectrogramChart() {
        ...
        let rs = SCIFastUniformHeatmapRenderableSeries()
        rs.dataSeries = spectrogramDS
        rs.minimum = minDB
        rs.maximum = maxDB
        
        rs.colorMap = SCIColorMap(colors: [.clear, UIColor.fromARGBColorCode(0xFF00008B), .purple, .red, .yellow, .white], andStops: [0, 0.0001, 0.25, 0.50, 0.75, 1])
        
        SCIUpdateSuspender.usingWith(spectrogramChart) {
            self.spectrogramChart.xAxes.add(xAxis)
            self.spectrogramChart.yAxes.add(yAxis)
            self.spectrogramChart.renderableSeries.add(rs)
        }
    }
}
