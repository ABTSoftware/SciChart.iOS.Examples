...
class VitalSignsMonitorChartView: SCDVitalSignsMonitorLayout {
    ...
    private func setupIndicators() {
        ...
        let bpBarStyle = SCDProgressBarStyle(isVertical: false, max: 10, spacing: 2, progressBackgroundColor: progressBackgroundColor, progressColor: UIColor.fromABGRColorCode(0xFF42B649), barSize: 3)
        
        bpBar.setStyle(bpBarStyle)
        
        let svBarStyle = SCDProgressBarStyle(isVertical: true, max: 12, spacing: 2, progressBackgroundColor: .clear, progressColor: UIColor.fromABGRColorCode(0xFFB0C4DE), barSize: 3)
    ...
    private func generateScatterForLastAppendedPoint(yAxisId: String, dataSeries: ISCIDataSeries) -> ISCIRenderableSeries {
        ...
        scatterSeries.pointMarker = EllipsePointMarker(size: 4, strokeStyle: SCISolidPenStyle(color: .white, thickness: 1), fillStyle: SCISolidBrushStyle(color: .white))
