...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-(0)-[SciChart]-(0)-|" options:0 metrics:0 views:layout]];
...
-(void) initializeSurfaceData{
    ...
    [_pieChartSurface setHoleRadius:100];
...
- (void)changeSwitch:(id)sender{
    if([sender isOn]){
        donutChart.drawLabels = YES;
        [_pieChartSurface invalidateElement];
    } else{
        donutChart.drawLabels = NO;
        [_pieChartSurface invalidateElement];
    }
}
