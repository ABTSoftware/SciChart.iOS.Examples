...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    SCDExampleViewController *exampleViewController = [[SCDExampleViewController alloc] initWithExample:[self p_SCD_exampleAt:indexPath] isSwift:_isSwift];
...
- (void)p_SCI_filterContentForSearchText:(NSString *)searchText scope:(NSString *)scope {
    ...
    for (id key in _dataSource.examples) {
        NSMutableArray<SCDExampleItem *> *item = [_dataSource.examples valueForKey:key];
        NSArray *filteredCategory = [item filteredArrayUsingPredicate:resultPredicate];
        [result addObjectsFromArray:filteredCategory];
    }
    _filteredExamples = result;
