...
class UsePaletteProviderView: SingleChartLayout {
    ...
    override func initExample() {
        ...
        mountainSeries.paletteProvider = XyCustomPaletteProvider(color: UIColor.red, annotation: boxAnnotation)
        
        let ellipsePointMarker = SCIEllipsePointMarker()
        ellipsePointMarker.fillStyle = SCISolidBrushStyle(color: UIColor.red)
        ellipsePointMarker.strokeStyle = SCISolidPenStyle(color: UIColor.orange, thickness: 2.0)
        ...
        lineSeries.paletteProvider = XyCustomPaletteProvider(color: UIColor.red, annotation: boxAnnotation)
        
        let ohlcSeries = SCIFastOhlcRenderableSeries()
        ohlcSeries.dataSeries = ohlcDataSeries
        ohlcSeries.paletteProvider = OhlcCustomPaletteProvider(color: UIColor.blue, annotation: boxAnnotation)
        
        let candlestickSeries = SCIFastCandlestickRenderableSeries()
        candlestickSeries.dataSeries = candlestickDataSeries
        candlestickSeries.paletteProvider = OhlcCustomPaletteProvider(color: UIColor.green, annotation: boxAnnotation)
        
        let columnSeries = SCIFastColumnRenderableSeries()
        columnSeries.dataSeries = columnDataSeries
        columnSeries.strokeStyle = SCISolidPenStyle(color: UIColor.blue, thickness: 1)
        columnSeries.zeroLineY = 6000
        columnSeries.fillBrushStyle = SCISolidBrushStyle(color: UIColor.blue)
        columnSeries.paletteProvider = XyCustomPaletteProvider(color: UIColor.purple, annotation: boxAnnotation)
        
        let squarePointMarker = SCISquarePointMarker()
        squarePointMarker.fillStyle = SCISolidBrushStyle(color: UIColor.red)
        squarePointMarker.strokeStyle = SCISolidPenStyle(color: UIColor.orange, thickness: 2.0)
        ...
        scatterSeries.paletteProvider = XyCustomPaletteProvider(color: UIColor.green, annotation: boxAnnotation)
