...
//
// SCSStackedBarChartView.swift is part of the SCICHART® Examples. Permission is hereby granted
...
class SCSStackedBarChartView: SingleChartLayout {
    ...
    override func initExample() {
        ...
        for i in 0..<yValues1.count {
            ds1.appendX(SCIGeneric(i), y: SCIGeneric(CDouble(yValues1[i])))
            ds2.appendX(SCIGeneric(i), y: SCIGeneric(CDouble(yValues2[i])))
            ds3.appendX(SCIGeneric(i), y: SCIGeneric(CDouble(yValues3[i])))
        }
     
        let columnCollection = SCIVerticallyStackedColumnsCollection()
        columnCollection.add(getRenderableSeriesWith(dataSeries: ds1, startColor: 0xff567893, endColor:0xff3D5568))
        columnCollection.add(getRenderableSeriesWith(dataSeries: ds2, startColor: 0xffACBCCA, endColor:0xff439AAF))
        columnCollection.add(getRenderableSeriesWith(dataSeries: ds3, startColor: 0xffDBE0E1, endColor:0xffB6C1C3))
        ...
        SCIUpdateSuspender.usingWithSuspendable(surface) {
            ...
            self.surface.chartModifiers = SCIChartModifierCollection(childModifiers: [xDragModifier, yDragModifier, SCIPinchZoomModifier(), SCIZoomExtentsModifier(), SCICursorModifier()])
    ...
    fileprivate func getRenderableSeriesWith(dataSeries: SCIXyDataSeries, startColor: uint, endColor: uint) -> SCIStackedColumnRenderableSeries {
        ...
        renderableSeries.fillBrushStyle = SCILinearGradientBrushStyle(colorCodeStart: startColor, finish: endColor, direction: .horizontal)
        renderableSeries.strokeStyle = SCISolidPenStyle(color: UIColor.black, withThickness: 0.5)
