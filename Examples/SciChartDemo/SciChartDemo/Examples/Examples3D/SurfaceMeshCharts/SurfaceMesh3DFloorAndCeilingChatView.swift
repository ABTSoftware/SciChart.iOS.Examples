...
class SurfaceMesh3DFloorAndCeilingChatView: SingleChartLayout3D {
    ...
    override func initExample() {
        ...
        rs1.dataSeries = ds
        rs1.drawMeshAs = .solidWireframe
        rs1.stroke = 0xFF228B22
        rs1.strokeThickness = 2.0
        rs1.maximum = 4
        rs1.drawSkirt = false
        rs1.meshColorPalette = palette
        rs1.opacity = 0.9

        let rs2 = SCISurfaceMeshRenderableSeries3D()
        rs2.dataSeries = ds
        rs2.heightScaleFactor = 0.0
        rs2.drawMeshAs = .solidWireframe
        rs2.stroke = 0xFF228B22
        rs2.strokeThickness = 2.0
        rs2.maximum = 4
        rs2.yOffset = 400
        rs2.meshColorPalette = palette
        rs2.opacity = 0.7
        ...
        SCIUpdateSuspender.usingWith(surface) {
            ...
            self.surface.renderableSeries.add(items: rs0, rs1, rs2)
