...
private let YAngle: Double = Double(GLKMathDegreesToRadians(-65))
private let CosYAngle: Double = cos(YAngle)
private let SinYAngle: Double = sin(YAngle)

class SeriesTooltip3DChartView: SingleChartLayout3D {
    ...
    override func initExample() {
        ...
        rs.strokeThickness = 4.0
        rs.pointMarker = pointMarker
        rs.isLineStrips = false
        ...
        tooltipModifier.crosshairPlanesFill = UIColor.fromARGBColorCode(0x33FF6600)
    ...
    private func appendPoint(_ ds: SCIXyzDataSeries3D, x: Double, y: Double, currentAngle: Double) {
        let radAngle = GLKMathDegreesToRadians(Float(currentAngle))
