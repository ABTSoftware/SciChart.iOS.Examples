...
class SurfaceMeshWithMetaDataProvider3DChartView: SingleChartLayout3D {
    ...
    override func initExample() {
        ...
        let rs0 = SCISurfaceMeshRenderableSeries3D()
        rs0.dataSeries = meshDataSeries0
        rs0.drawMeshAs = .solidMesh
        rs0.drawSkirt = false
        rs0.meshColorPalette = palette0
        rs0.metadataProvider = SurfaceMeshMetaDataProvider3D()
        
        let rs1 = SCISurfaceMeshRenderableSeries3D()
        rs1.dataSeries = meshDataSeries1
        rs1.drawMeshAs = .solidMesh
        rs1.drawSkirt = false
        rs1.meshColorPalette = palette1
        rs1.metadataProvider = SurfaceMeshMetaDataProvider3D()
        
        SCIUpdateSuspender.usingWith(surface) {
            self.surface.xAxis = xAxis
            self.surface.yAxis = yAxis
            self.surface.zAxis = zAxis
            self.surface.renderableSeries.add(items: rs0, rs1)
