...
//        }
    ...
    func testGenerateScreenshots() {
        ...
        snapshot("1-Uniform-Mesh-3D", timeWaitingForIdle: timeWaitingFor3D)
        app.navigationBars["Uniform Mesh Chart 3D"].buttons["3D CHARTS"].tap()
        
        // Ellipsoid
        app.tables.staticTexts["Ellipsoid Free Surface Chart 3D"].tap()
        snapshot("2-Ellipsoid-3D", timeWaitingForIdle: timeWaitingFor3D)
        ...
        snapshot("3-Waterfall-3D", timeWaitingForIdle: timeWaitingFor3D)
        app.navigationBars["Waterfall Chart 3D"].buttons["3D CHARTS"].tap()
        
        // Logarithmic Axis
        app.tables.staticTexts["Logarithmic Axis 3D"].tap()
        app.children(matching: .window).element(boundBy: 0).swipeDown()
        snapshot("4-Logarithmic-Axis-3D", timeWaitingForIdle: timeWaitingFor3D)
        ...
        snapshot("5-Candlestick", timeWaitingForIdle: timeWaitingFor2D)
        ...
        snapshot("6-Vertically-Stacked-Axes", timeWaitingForIdle: timeWaitingFor2D)
        app.navigationBars["Vertically Stacked Axes"].buttons["2D CHARTS"].tap()
        
        // Scatter
        app.tables.staticTexts["Scatter Chart"].tap()
        snapshot("7-Scatter", timeWaitingForIdle: timeWaitingFor2D)
