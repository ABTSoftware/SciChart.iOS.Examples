...
class DefaultVitalSignsDataProvider: DataProviderBase<VitalSignsData> {
    ...
    init() {
        ...
        do {
            let rawData = try String.init(contentsOfFile: getBundleFilePath(from: ECG_TRACES), encoding: .utf8)
    ...
    private func getBundleFilePath(from path: String) -> String {
        let components = path.components(separatedBy: ".")
        let fileName = components[0]
        let extensionName = components[1]
        
        let filePath = Bundle.main.path(forResource: fileName, ofType: extensionName)!
        return filePath
    }
