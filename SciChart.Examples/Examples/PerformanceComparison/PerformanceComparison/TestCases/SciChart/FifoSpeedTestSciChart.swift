...
//
// FifoSpeedTestSciChart.swift is part of the SCICHART® PerformanceComparison.
...
class FifoSpeedTestSciChart: SciChartSpeedTestBase {
    ...
    override func initChart(testParameters: TestParameters) {
        dataSeries.fifoCapacity = testParameters.pointCount
        
        let randomWalkData = randomWalkGenerator.getRandomWalkSeries(count: testParameters.pointCount - 1, min: 0.0, max: 1.0)
        let xValues = SCIDoubleValues(capacity: randomWalkData.count)
        let yValues = SCIDoubleValues(capacity: randomWalkData.count)
        
        for i in 0 ..< randomWalkData.count {
            xValues.add(randomWalkData[i].x)
            yValues.add(randomWalkData[i].y)
        }
        xCount = randomWalkData.count
        
        SCIUpdateSuspender.usingWith(self.surface) {
            self.dataSeries.append(x: xValues, y: yValues)
        }
    }
    
    override func updateChart(testParameters: TestParameters) {
        dataSeries.append(x: xCount, y: randomWalkGenerator.next(min: 0.0, max: 1.0))        
        xCount += 1
    }
}
