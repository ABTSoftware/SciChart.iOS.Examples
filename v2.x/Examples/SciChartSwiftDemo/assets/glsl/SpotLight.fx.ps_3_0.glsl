...
void main ()
{
  ...
  tmpvar_24 = (v_20.xyz - worldPosition_17.xyz);
  highp vec3 tmpvar_25;
  tmpvar_25 = -(v_22.xyz);
  highp float tmpvar_26;
  tmpvar_26 = clamp ((1.0 - (v_23.w * 
    dot (tmpvar_24, tmpvar_24)
  )), 0.0, 1.0);
  attenuation_19 = (tmpvar_26 * tmpvar_26);
  highp float tmpvar_27;
  tmpvar_27 = clamp (((v_23.y * v_23.z) - (
    clamp (dot (tmpvar_25, normalize(tmpvar_24)), 0.0, 1.0)
   * v_23.z)), 0.0, 1.0);
  attenuation_19 = (attenuation_19 * (tmpvar_27 * tmpvar_27));
  attenuation_19 = (attenuation_19 * v_21.w);
  result_2.xyz = (((
    (attenuation_19 * tmpvar_6)
   * 
    max (dot (tmpvar_8, tmpvar_25), 0.0)
  ) + (
    (attenuation_19 * tmpvar_7)
   * 
    pow (clamp (dot ((tmpvar_25 - 
      (2.0 * (dot (tmpvar_8, tmpvar_25) * tmpvar_8))
    ), -(
