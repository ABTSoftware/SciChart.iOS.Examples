...
@interface CustomTicks : SCINumericTickProvider

@end

@implementation CustomTicks

-(SCIArrayController *)calculateMajorTicksWithRange:(id<SCIRangeProtocol>)tickRange Delta:(id<SCIAxisDeltaProtocol>)tickDelta {
    SCIArrayController * ticks = [[SCIArrayController alloc] initWithType:SCIDataType_Double];
...
-(void) addAxes{
    ...
    axis.axisId = @"yAxis";
    axis.tickProvider = [CustomTicks new];
    axis.style.minorTickBrush = [[SCISolidPenStyle alloc] initWithColor:[UIColor whiteColor] withThickness:1];
