...
@interface CustomDeltaCalculator : SCINumericDeltaCalculator

@end

@implementation CustomDeltaCalculator
...
-(id<SCIAxisDeltaProtocol>)getDeltaFromRangeMin:(SCIGenericType)min Max:(SCIGenericType)max MinorsPerMajor:(int)minorsPerMajor MaxTicks:(uint)maxTicks {
    double minorDelta = 1;
    double majorDelta = 4;
    SCIDoubleAxisDelta * delta = [[SCIDoubleAxisDelta alloc] initWithMin:minorDelta Max:majorDelta];
    return delta;
}

@end

@interface CustomAxis : SCINumericAxis
...
-(id<SCIDeltaCalculatorProtocol>)getDeltaCalculator {
    return [CustomDeltaCalculator new];
}
...
-(void) addAxes{
    id<SCIAxis2DProtocol> axis = [CustomAxis new];
    axis.axisId = @"yAxis";
